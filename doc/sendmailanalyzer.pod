...
This is especially right when with postfix configured to have local delivery
via dovecot service. Default: dovecot, that means that recipient log lines
with relay=dovecot will instruct sendmailanalyzer to skip those messages.
...
Copyright (c) 2002-2014 Gilles Darold - All rights reserved.
